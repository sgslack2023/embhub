...
class PDFProcessor:
    ...
    def extract_shipping_address(self, text: str) -> str:
        ...
        try:
            ...
            # Simplified and more robust patterns for shipping address extraction
            address_patterns = [
                # Pattern 1: Everything after "SHIP TO:" until next section
                r'SHIP\s*TO\s*:?\s*(.*?)(?=\n\s*(?:USPS|TRACKING|PRIORITY|FROM|Delivery|Return|Service)|\Z)',
                
                # Pattern 2: Everything after "TO:" 
                r'(?:^|\n)\s*TO\s*:?\s*(.*?)(?=\n\s*(?:USPS|TRACKING|PRIORITY|FROM|Delivery|Return|Service)|\Z)',
                
                # Pattern 3: Simple 3-line address (most common)
                r'([A-Za-z][^\n]{8,}\n[^\n]*\d+[^\n]{5,}\n[^\n]*[A-Z]{2}\s+\d{5})',
                
                # Pattern 4: Any text block with name, street number, and ZIP
                r'([A-Za-z][A-Za-z\s]{3,}[^\n]*\n.*?\d+.*?\n.*?[A-Z]{2}\s+\d{5}(?:-\d{4})?)',
                
                # Pattern 5: Flexible address with ZIP (work backwards from ZIP)
                r'([^\n]*\n[^\n]*\n[^\n]*[A-Z]{2}\s+\d{5}(?:-\d{4})?)',
                
                # Pattern 6: Very loose - any multi-line text with numbers and letters
                r'([A-Za-z][^\n]{10,}\n[^\n]{10,}\n[^\n]{10,})',
            ]
            
            for i, pattern in enumerate(address_patterns, 1):
            ...
            for line in lines:
                ...
                if any(skip in line.upper() for skip in ['USPS', 'PRIORITY', 'MAIL', 'TRACKING', 'ATFM', 'POSTAGE']):
                    continue
    ...
    def find_best_matching_packing_slip(self, ocr_text: str, packing_slips: List[Dict]) -> Tuple[Optional[int], float]:
        ...
        print(f"=== SIMPLE ADDRESS MATCHING ===")
    ...
    def process_shipping_labels_pdf(self, pdf_path: str, packing_slips: List[Dict], google_drive_file_link: str) -> List[Dict]:
        ...
        try:
            ...
            for page_num, page_path in enumerate(page_paths, 1):
                ...
                print(f"Matching against {len(packing_slips)} packing slips from same folder...")
                ...
                print(f"Page {page_num} processed - Matched: {result['matched']}")
